...
MenuItem(miTest5, "Test 5 Menu", Menu::NullItem, miTest4,    miExit, Menu::NullItem, menuDummy);
...
void loop() {
  ...
  if (updateMenu) {
    ...
    uint8_t sbMarkSize = sbHeight * sbItems / mi.siblings;
    uint8_t sbMarkPos = ((sbHeight - sbMarkSize) / sbItems) * (mi.position - 1);
    tft.fillRect(sbLeft, sbTop, sbWidth, sbHeight, ST7735_WHITE);
    tft.fillRect(sbLeft, sbMarkPos, sbWidth, sbMarkSize, ST7735_RED);
